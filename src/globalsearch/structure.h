...
public:
  ...
  /** Functions to set or retrieve the "composition group index"
   *  for a structure. The group index identifies which independent
   *  chemical system (CompGroup) this structure belongs to in a
   *  multi-composition search. Default is -1 (unset).
   */
  int  compositionGroupIndex() const {return m_compositionGroupIndex;};
  void setCompositionGroupIndex(int g) {m_compositionGroupIndex = g;};
...
protected:
  ...
  // Composition group index: identifies which CompGroup this structure
  // belongs to in a multi-composition search. -1 = unset.
  int m_compositionGroupIndex;
