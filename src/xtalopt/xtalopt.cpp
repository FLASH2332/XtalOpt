...
Xtal* XtalOpt::generateNewXtal(CellComp incomp)
{
  ...
  // In multi-composition searches, parent pool is filtered for the specific group
  int groupIdx = findCompGroup(incomp);
  if (groupIdx != -1) {
    structures = getParentPoolForGroup(groupIdx);
  } else {
    ...
    while (attemptCount < 1000 && !checkXtal(xtal)) {
      ...
      switch (op) {
        case OP_Crossover: {
          ...
          // Determine composition list for the parent's group
          int groupIdx = selectedXtal ? selectedXtal->compositionGroupIndex() : -1;
          const QList<CellComp>& grpComps = (groupIdx >= 0 && groupIdx < compGroups.size())
                                          ? compGroups[groupIdx].comps : this->compList;

          // Perform operation
          xtal = XtalOptGenetic::crossover(xtal1, xtal2, grpComps, this->eleMinRadii,
        ...
        case OP_Permutomic: {
          ...
          const QList<CellComp>& grpComps = (groupIdx >= 0 && groupIdx < compGroups.size())
                                          ? compGroups[groupIdx].comps : this->compList;
          CellComp refComp = !grpComps.isEmpty() ? grpComps[0] : CellComp();

          xtal = XtalOptGenetic::permutomic(selectedXtal, refComp, this->eleMinRadii,
        ...
        case OP_Permucomp: {
          ...
          xtal = XtalOptGenetic::permucomp(selectedXtal, refComp, this->eleMinRadii,
                                           minAtoms, maxAtoms, m_verbose);
...
bool XtalOpt::checkComposition(Xtal* xtal, bool isSeed)
{
  ...
  if (compGroups.isEmpty() && compList.isEmpty()) {
  ...
  // Find which composition group this xtal belongs to
  int groupIdx = findCompGroup(comp);

  // If not an exact element-set match (e.g. off-composition seed), try finding a group containing all symbols
  if (groupIdx == -1) {
    for (int g = 0; g < compGroups.size(); g++) {
      bool match = true;
      for (const auto& sym : symbols) {
        if (!compGroups[g].chemSystem.contains(sym)) {
          match = false;
          break;
        }
      }
  ...
  if (groupIdx == -1) {
    qDebug() << "Error checkComposition: xtal elements" << symbols
             << "do not match any defined composition group.";
    return false;
  }

  // Tag the xtal permanently with its composition group index
  xtal->setCompositionGroupIndex(groupIdx);

  const CompGroup& grp = compGroups[groupIdx];
  const QList<QString>& chemSystem = grp.chemSystem;
  const QList<CellComp>& grpComps = grp.comps;

  // Perform checks within the matched group
  ...
  // Is there any species not defined in the group's chemical system?
  ...
  // Is there any species of the group's chemical system that is missing?
  for (int i = 0; i < chemSystem.size(); i++) {
    if (!symbols.contains(chemSystem[i]))
      hasMissingTypes = true;
  }

  // Is chemical composition equivalent to any on the group's formula list?
  for (int i = 0; i < grpComps.size(); i++) {
    if (compareCompositions(grpComps[i], comp) != 0.0) {
      compositionIsNew = false;
      break;
    }
  }

  // Having extra types not in group system is always an error
  if (hasExtraTypes) {
    qDebug() << "Error checkComposition: unknown types in xtal for group" << groupIdx;
    return false;
  }

  // Seed structures handling
  if (isSeed) {
  ...
  if (hasMissingTypes) {
    qDebug() << "Error checkComposition: atomic types missing for group" << groupIdx;
    return false;
  }

  if (hasExtraAtomCount) {
    qDebug() << "Error checkComposition: atom count" << numAtoms << "exceeds maxAtoms" << maxAtoms;
    return false;
  }

  if (hasLowAtomCount) {
    qDebug() << "Error checkComposition: atom count" << numAtoms << "is below minAtoms" << minAtoms;
    return false;
  }
  ...
  if (compositionIsNew) {
    qDebug() << "Error checkComposition: composition doesn't match any formula in group" << groupIdx;
    return false;
  }
...
std::vector<double> XtalOpt::getReferenceEnergiesVector(int groupIdx) const
{
  std::vector<double> out;
  if (groupIdx < 0 || groupIdx >= compGroups.size())
    return out;

  const CompGroup& grp = compGroups[groupIdx];

  for (int i = 0; i < grp.refEnergies.size(); i++) {
    QList<QString> symbols = grp.refEnergies[i].cell.getSymbols();
    double energy = grp.refEnergies[i].energy;
    for (int j = 0; j < grp.chemSystem.size(); j++) {
      double natoms = 0.0;
      int ind = symbols.indexOf(grp.chemSystem[j]);
      if (ind != -1) {
        natoms = static_cast<double>(grp.refEnergies[i].cell.getCount(symbols[ind]));
      }
      out.push_back(natoms);
    }
    out.push_back(energy);
...
{
  QList<QString> sys;
  for (const auto& grp : compGroups) {
    for (const auto& sym : grp.chemSystem) {
      if (!sys.contains(sym))
        sys.append(sym);
    }
  }

  if (sys.isEmpty() && !compList.isEmpty()) {
    sys = compList[0].getSymbols();
    for (int i = 1; i < compList.size(); i++) {
      for (const auto& sym : compList[i].getSymbols()) {
        if (!sys.contains(sym))
          sys.append(sym);
      }
    }
...
void XtalOpt::updateHullAndFrontInfo()
{
  if (compGroups.size() <= 1) {
    SearchBase::updateHullAndFrontInfo();
    return;
  }

  // Multi-composition run: calculate convex hull PER COMPOSITION GROUP
  QList<Structure*> structures = m_queue->getAllStructures();
  if (structures.isEmpty())
    return;

  for (int g = 0; g < compGroups.size(); ++g) {
    const CompGroup& grp = compGroups[g];
    const QList<QString>& eleSymb = grp.chemSystem;
    int eleNumb = eleSymb.size();
    int chlNdim = eleNumb + 1;

    QList<Structure*> groupStructures;
    std::vector<double> chlData;
    ...
    for (int i = 0; i < structures.size(); i++) {
      ...
      // Group filter: check tag or group index
      int sGroup = s->compositionGroupIndex();
      if (sGroup == -1) {
        sGroup = findCompGroup(getXtalComposition(s));
        if (sGroup != -1)
          s->setCompositionGroupIndex(sGroup);
      }

      if (sGroup != g)
        continue;

      groupStructures.append(s);

      double enth = s->getEnthalpy();
      for (int j = 0; j < eleNumb; j++) {
        chlData.push_back((double)s->getNumberOfAtomsOfSymbol(eleSymb[j]));
      }
      chlData.push_back(enth);
    }

    if (groupStructures.isEmpty())
      continue;

    // Append reference energies for group g
    std::vector<double> refData = getReferenceEnergiesVector(g);
    int refNumb = refData.size() / (eleNumb + 1);
    for (int i = 0; i < refNumb; i++) {
      for (int j = 0; j < eleNumb + 1; j++)
        chlData.push_back(refData[i * (eleNumb + 1) + j]);
    }

    int strNumb = groupStructures.size();
    int chlNumb = strNumb + refNumb;

    std::vector<double> strHull(chlNumb);
    if (!distAboveHull(chlData, chlNumb, chlNdim, strHull)) {
      qDebug() << "Warning: distAboveHull failed for composition group" << g;
      continue;
    }

    // Assign distance above hull to structures in group g
    for (int i = 0; i < strNumb; i++) {
      QWriteLocker lock(&groupStructures[i]->lock());
      groupStructures[i]->setDistAboveHull(strHull[i]);
    }
  }
...
CellComp XtalOpt::getMinimalComposition(int groupIdx)
{
  ...
  // If groupIdx >= 0, operate on that group's comps; otherwise use global compList.

  CellComp compMins;

  const QList<CellComp>& list = (groupIdx >= 0 && groupIdx < compGroups.size())
                                ? compGroups[groupIdx].comps : compList;

  if (list.isEmpty())
    return compMins;

  QList<QString> symbols = list[0].getSymbols();
  QList<uint> counts;
  for (int i = 0; i < symbols.size(); i++)
    counts.append(list[0].getCount(symbols[i]));

  for (int i = 1; i < list.size(); i++) {
    for (int j = 0; j < symbols.size(); j++) {
      if (!list[i].getSymbols().contains(symbols[j]))
        return compMins;
      uint c = list[i].getCount(symbols[j]);
      if (c < counts[j])
        counts[j] = c;
    }
  }
...
CellComp XtalOpt::getMaximalComposition(int groupIdx)
{
...
int XtalOpt::findCompGroup(const CellComp& comp) const
{
  QList<QString> symbols = comp.getSymbols();
  for (int g = 0; g < compGroups.size(); g++) {
    if (compGroups[g].chemSystem == symbols)
      return g;
  }
  return -1;
}

QList<GlobalSearch::Structure*> XtalOpt::getParentPoolForGroup(int groupIdx) const
{
  QList<GlobalSearch::Structure*> filtered;
  if (!m_queue)
    return filtered;

  QList<GlobalSearch::Structure*> all = m_queue->getAllParentPoolStructures();
  for (auto* s : all) {
    if (s && s->compositionGroupIndex() == groupIdx)
      filtered.append(s);
  }
  return filtered;
}
...
{
  // Processes input chemical formulas. Multiple composition groups (chemical systems)
  // can be separated by a semicolon ';'. Within each group, formulas are separated by ','.
  // Example: "LiNiO2, Li2NiO4 ; LiAlO2, Li2AlO3"
  ...
  input_formulas_string = s;

  QList<CompGroup> newGroups;
  QList<CellComp>  newFlatCompList;

  // Split input by semicolon to separate composition groups
  QStringList groupStrings = s.split(';', QString::SkipEmptyParts);
  if (groupStrings.isEmpty()) {
    qDebug() << "Error processInputChemicalFormulas: empty formula string provided.";
    return false;
  }

  for (int g = 0; g < groupStrings.size(); ++g) {
    QString groupStr = groupStrings[g].trimmed();
    QStringList formulalist = groupStr.split(',');

    QList<CellComp> groupComps;
    ...
    for (const auto &tmpform : qAsConst(formulalist)) {
      ...
      if (!formula.contains("-")) {
        ...
        if (tmpcomp.getNumTypes() > 0) {
          groupComps.append(tmpcomp);
          continue;
        } else {
          qDebug() << "Error processInputChemicalFormulas: incorrect formula '"
                   << formula << "' in group" << g;
          return false;
        }
      }

      // Range formula "comp1 - comp2"
      QStringList expcomp = formula.split("-");
      if (expcomp.size() != 2) {
        qDebug() << "Error processInputChemicalFormulas: invalid range formula '"
                 << formula << "' in group" << g;
        return false;
      }
      ...
      if (comp1.getNumTypes() == 0 || comp2.getNumTypes() == 0) {
        qDebug() << "Error processInputChemicalFormulas: failed parsing range entry '"
                 << formula << "' in group" << g;
        return false;
      }
      ...
      if (ratio == 0 || ratio != std::floor(ratio) || ratio < 1) {
        qDebug() << "Error processInputChemicalFormulas: range end is not a valid supercell in '"
                 << formula << "' in group" << g;
        return false;
      }
      ...
      for (uint i = 1; i <= quantratio; i++) {
        ...
        groupComps.append(formulaToComposition(frm));
      }
    }

    if (groupComps.isEmpty()) {
      qDebug() << "Error processInputChemicalFormulas: group" << g << "has no valid formulas.";
      return false;
    }

    // Verify all formulas within this group share the exact same element types
    QList<QString> firstSymbols = groupComps[0].getSymbols();
    for (int i = 1; i < groupComps.size(); ++i) {
      if (groupComps[i].getSymbols() != firstSymbols) {
        qDebug() << "Error processInputChemicalFormulas: formulas within group" << g
                 << "must share the exact same elements. Got" << groupComps[i].getSymbols()
                 << "vs" << firstSymbols;
        return false;
      }
    }
    ...
    CompGroup grp;
    grp.comps = groupComps;
    grp.chemSystem = firstSymbols; // CellComp::getSymbols() is already sorted alphabetically
    newGroups.append(grp);

    newFlatCompList.append(groupComps);
  }

  // Update members
  compGroups = newGroups;
  compList = newFlatCompList;

  // Rebuild global eleMinRadii covering the UNION of all elements from all groups
  eleMinRadii.clear();
  for (const auto& grp : compGroups) {
    for (const auto& sym : grp.chemSystem) {
      uint atomcn = ElementInfo::getAtomicNum(sym.toStdString());
      if (eleMinRadii.getMinRadius(atomcn) < 1e299)
...
bool XtalOpt::processInputReferenceEnergies(QString s)
{
  ...
  // If no non-empty entries are given, construct default "zero" value for all elements.
  if (nonEmptyEntries == 0) {
    ...
    // If user gives ref energies, at least one of each element must be given.
    for (int i = 0; i < chemSystem.size(); i++) {
      if (eleRefs[i] == 0) {
  ...
  // Initiate main reference energy global variable
  refEnergies = output_list;

  // Assign reference energies to each CompGroup (for per-group hull calculations)
  for (auto& grp : compGroups) {
    grp.refEnergies.clear();
    for (const auto& ref : output_list) {
      QList<QString> refSymbols = ref.cell.getSymbols();
      bool allInGroup = true;
      for (const auto& sym : refSymbols) {
        if (!grp.chemSystem.contains(sym)) {
          allInGroup = false;
          break;
        }
      }
      if (allInGroup) {
        grp.refEnergies.append(ref);
      }
    }

    // Ensure every element in grp.chemSystem has an elemental reference in grp.refEnergies
    for (const auto& sym : grp.chemSystem) {
      bool hasEleRef = false;
      for (const auto& ref : grp.refEnergies) {
        if (ref.cell.getNumTypes() == 1 && ref.cell.getSymbols().contains(sym)) {
          hasEleRef = true;
          break;
        }
      }
      if (!hasEleRef) {
        QString frm = sym + "1";
        CellComp comp = formulaToComposition(frm);
        grp.refEnergies.append({comp, 0.0});
      }
    }
  }
